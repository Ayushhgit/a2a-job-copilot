...
class ResumeVectorStore:
    ...
    def load_user(self, profile: Any):
        ...
        for idx, exp in enumerate(profile.experience):
            ...
            self.registry[self._current_id] = {"type": "experience", "data": exp.dict()}
            self._current_id += 1
        ...
        for idx, proj in enumerate(profile.projects):
            ...
            embed = self.model.encode([text])[0]
            self.index.add(np.array([embed]).astype('float32'))
            self.registry[self._current_id] = {"type": "project", "data": proj.dict()}
            self._current_id += 1
    ...
    def search(self, query: str, top_k: int = 5) -> Dict[str, List[Any]]:
        ...
        for idx in I[0]:
            if idx != -1 and idx in self.registry:
                item = self.registry[idx]
                results[item["type"]].append(item["data"])
